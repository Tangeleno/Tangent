Tangent (Tangeleno's Agent Network for Game Environment and Tactical Navigation) is a Cooperative Bot system for the game MMORPG EverQuest using MacroQuest to interface with the game
...
1. Learning Modules - Likely Deep Q-Learning, the state is complex and currently the number of inputs aren't well defined but they could be large
    1. Agent Learning will be done for each agent archtype (healer, dps, tank, cc) and merged with learning for each class (details on how to do this are fuzzy). The learning will adjust the utility weights and activation scores of the utility AI
    1. Coordinator Learning will be done for all teams as a baseline and then specificly for each team. The learning will adjust the cost estimations, and action selection, with possible future state effecting world state representation

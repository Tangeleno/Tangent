...
function MemorizeSpellNode.new(args)
    mq.Write.Trace("%s: Creating MemorizeSpellNode with spellGemKey '%s' and spellIdKey '%s'", args.name, args.spellGemKey, args.spellIdKey)
    ...
    function self._OnInitialize(blackboard)
        ...
        mq.Write.Debug("%s: Initialized with spell '%s' (ID: %s) to memorize in gem slot %s", self.Name, spellName, self.spellId, self.spellGem)
    end
    ...
    function self._Update(blackboard)
        ...
        if self.memStarted and self.memWaitTimer < 1500 then
            ...
            mq.Write.Debug("%s: Waiting for spell '%s' memorization, timer at %d ms", self.Name, spell.Name(), self.memWaitTimer)
            return NodeState.Running
        end
